...
@section('content')
    ...
    <div class="card card-body border-0 shadow">
        <form action="{{ route('accessibilities.store') }}" method="POST">
            @csrf
            ...
            <div class="mb-3">
                ...
                <select name="position" class="form-select @error('position') is-invalid @enderror">
                    ...
                    @for ($i = 1; $i <= 3; $i++)
                        <option value="{{ $i }}" @selected((int) old('position') === $i)>{{ $i }}</option>
                    @endfor
                </select>
                @error('position')
            ...
            <div class="mb-3">
                ...
                <textarea name="title" rows="4" class="form-control @error('title') is-invalid @enderror">{{ old('title') }}</textarea>
            ...
            <div class="mb-3">
                ...
                <input type="text" name="url" class="form-control @error('url') is-invalid @enderror"
                    value="{{ old('url') }}" placeholder="/some-page or https://example.com or #">
                @error('url')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
